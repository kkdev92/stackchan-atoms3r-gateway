...
.DESCRIPTION
    The application consumes SDK packages from local-nuget. When a package is rebuilt at the
...
Get-ChildItem (Join-Path $root 'local-nuget') -ErrorAction SilentlyContinue |
    Where-Object { $_.Extension -in '.nupkg', '.snupkg' } |
    Remove-Item -Force

dotnet pack (Join-Path $root 'src/sdk/StackChan.Gateway.Sdk.slnx') -c $Configuration `
    -o (Join-Path $root 'local-nuget')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Get-ChildItem (Join-Path $root '.packages') -Directory -Filter 'kkdev92.stackchan.*' -ErrorAction SilentlyContinue |
    Remove-Item -Recurse -Force

$packages = Get-ChildItem (Join-Path $root 'local-nuget') -Filter '*.nupkg'
$symbols = Get-ChildItem (Join-Path $root 'local-nuget') -Filter '*.snupkg'
...
Write-Host "packed  : $($packages.Count) nupkg + $($symbols.Count) snupkg -> local-nuget"
